...
//import SimpleMarkdown from "../../libs/simple-markdown.js";
import SimpleMarkdown from "simple-markdown";
import TeX from "../tex.js";

const Util = { rWidgetRule: /^\[\[\u2603 (([a-z-]+) ([0-9]+))\]\]/ };
...
var rules = _.extend({}, SimpleMarkdown.defaultRules, {
    ...
    blockMath: {
        ...
        react: (node, output, state) => {
            ...
            return (
                <TeX key={state.key} displaystyle>
                    {node.content}
                </TeX>
            );
        }
    ...
    lint: {
        ...
        react: (node, output, state) => {
            console.log(
                "perseus-markdown lint triggered",
                node.message,
                node.ruleName,
                node.severity
            );
...
function ruleOverrideOutput(newRules = {}) {
    const extendedRules = { ...rules };
    for (const [key, val] of Object.entries(newRules)) {
        extendedRules[key] = extendedRules[key] || {};
        extendedRules[key] = { ...extendedRules[key], ...val };
    }
    return SimpleMarkdown.reactFor(
        SimpleMarkdown.ruleOutput(extendedRules, "react")
    );
}

const { ruleOutput, basicOutput, sanitizeUrl, reactFor, parseInline } = {
